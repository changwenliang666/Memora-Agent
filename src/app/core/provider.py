...
from app.schema.config import ProviderConfig
...
class LLMProvider:
    ...
    def get_model(self, provider_type: str, model_name: str) -> BaseChatModel:
        ...
        if provider.type == "openai":
            return ChatOpenAI(
                api_key=self._openai_api_key(provider, provider_type),
                base_url=provider.base_url,
                model=model_name,
                temperature=provider.temperature,
                extra_body={
        ...
        raise ValueError(f"不支持的供应商类型 {provider.type}（{provider_type}）")

    def get_embeddings(self, provider_type: str, model_name: str) -> Embeddings:
        provider = self._get_provider(provider_type)
    ...
    def _openai_api_key(self, provider: ProviderConfig, provider_type: str) -> str:
        if not provider.api_key_env:
            raise ValueError(f"Provider {provider_type} 未配置 api_key_env")
